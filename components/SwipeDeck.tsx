  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden p-4 md:p-8 perspective-1000 flex-col">
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                ...
                <div className="space-y-3 mb-6">
                  {REPORT_REASONS.map((reason) => (
                    <label key={reason} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${reportReason === reason ? 'bg-primary/20 border-primary' : 'bg-background border-white/5 hover:bg-white/5'}`}>
                      <input type="radio" name="reportReason" value={reason} checked={reportReason === reason} onChange={(e) => setReportReason(e.target.value)} className="hidden" />
                      <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${reportReason === reason ? 'border-primary' : 'border-text-muted'}`}>
                        {reportReason === reason && <div className="w-2 h-2 rounded-full bg-primary" />}
                      </div>
                      <span className={`text-sm ${reportReason === reason ? 'text-white font-medium' : 'text-text-muted'}`}>{reason}</span>
                    </label>
                  ))}
                </div>
                {reportReason === 'Other' && (
                  <div className="mb-6 animate-in fade-in slide-in-from-top-2">
                    ...
                    <textarea value={reportDetails} onChange={(e) => setReportDetails(e.target.value)} className="w-full bg-background border border-white/10 rounded-xl p-3 text-sm text-text-main focus:outline-none focus:border-gold/50 resize-none h-24" placeholder="Please tell us more..." />
                  </div>
                )}
                <div className="bg-white/5 rounded-xl p-4 border border-white/5 mb-6">
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" checked={blockChecked} onChange={(e) => setBlockChecked(e.target.checked)} className="mt-1 accent-gold" />
                    <div><span className="block text-sm font-bold text-text-main">Also block this user</span><span className="block text-xs text-text-muted mt-1 leading-relaxed">You won't see each other in Discover, matches, or chat.</span></div>
                  </label>
                </div>
              </div>
              <div className="flex flex-col gap-3 mt-2 pt-4 border-t border-white/10">
                <button onClick={handleReportSubmit} disabled={!reportReason || isSubmittingReport} className="w-full py-3 bg-primary text-white rounded-xl font-bold hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{isSubmittingReport ? "Submitting..." : "Submit Report"}</button>
                <div className="flex items-center gap-3">
                  <button onClick={handleCloseReportModal} className="flex-1 py-3 text-sm text-text-muted hover:text-white transition-colors">Cancel</button>
                  <button onClick={handleBlockOnly} className="flex-1 py-3 text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-colors flex items-center justify-center gap-2"><ShieldOff size={14} /> Block User</button>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      >
        ...
        {/* Reliability Badge (Card Face) */}
        {activeUser.reliabilityLabel && (
          <div 
            className="absolute top-14 left-4 z-40 bg-black/60 backdrop-blur-md border border-white/20 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-lg group/badge cursor-help"
            title="Based on attendance and responsiveness"
          >
            <Shield size={10} className="text-emerald-400" />
            {activeUser.reliabilityLabel}
            <div className="absolute left-0 top-full mt-2 w-max px-2 py-1 bg-black text-xs text-white rounded opacity-0 group-hover/badge:opacity-100 transition-opacity pointer-events-none">
               Based on attendance & responsiveness
            </div>
          </div>
        )}
      ...
      <div className="relative mt-6 w-full max-w-sm md:max-w-md flex justify-center items-center gap-3 md:gap-5 z-20">
          ...
          <button 
            className={`w-12 h-12 md:w-14 md:h-14 rounded-full border border-white/10 flex items-center justify-center shadow-lg backdrop-blur-sm transition-all relative ${history ? 'bg-surface text-text-muted hover:text-white hover:bg-white/10 hover:scale-105' : 'bg-surface/50 text-white/20 cursor-not-allowed'}`}
            onClick={handleUndo}
            disabled={!history && !isFree}
            title={isFree ? "Upgrade to Rewind" : `Rewind (${rewindsLeft} left)`}
          >
            {isFree ? (<div className="relative"><RotateCcw size={20} className="opacity-50" /><Lock size={10} className="absolute -top-1 -right-1 text-gold" /></div>) : (<RotateCcw size={20} />)}
          </button>
          ...
          <button 
            className={`w-14 h-14 md:w-16 md:h-16 rounded-full border flex items-center justify-center transition-all shadow-xl backdrop-blur-sm ${isSwipesExhausted ? 'bg-surface border-white/10 text-text-muted hover:bg-white/10 cursor-not-allowed opacity-70' : 'bg-surface border-emerald-500/30 text-emerald-500 hover:bg-emerald-500 hover:text-white hover:scale-110 hover:border-emerald-500'}`}
          ...
          <button 
            className={`relative w-12 h-12 md:w-14 md:h-14 rounded-full bg-surface border flex items-center justify-center transition-all shadow-lg backdrop-blur-sm group ${dailySuperLikes >= superLikesLimit || isSwipesExhausted ? 'border-white/10 text-text-muted hover:bg-white/5' : 'border-primary/30 text-primary hover:bg-primary hover:text-white hover:scale-105 hover:border-primary'}`}
